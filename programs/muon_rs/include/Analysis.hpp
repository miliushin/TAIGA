...
private:
    ...
    unsigned int NeventMUON_NEW[NUMBER_OF_MUON_ST];    // Number of events after coin
    char *rs_path;

    double GetTimeMUON(unsigned char ch[8]);
    int FindCoin_MUON_GRANDE(double Time);             // coincidences between the MUON and the GRANDE st.
    int FindCoin_MUON_MUON(double Time, int Nstation); // coincidences between a MUON and another MUON st.
