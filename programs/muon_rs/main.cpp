            ...
            while ((data_ent = readdir(data_dir)) != NULL) {
              ...
              if (data_ent->d_type == DT_DIR) {
                ...
                if (ST_DIR[0] == 'M' && ST_DIR[1] == 'u') {
                  ...
                  memset(mk_dir, '\0', 200);
                  sprintf(mk_dir, "cp -r %s/%s %s/%s", DATA_DIR,
                          data_ent->d_name, RS_DIR, data_ent->d_name);
                  system(mk_dir);
                }
              } else {
                memset(mk_dir, '\0', 200);
                sprintf(mk_dir, "cp -r %s/%s %s/%s", DATA_DIR, data_ent->d_name,
                        RS_DIR, data_ent->d_name);
                system(mk_dir);
              }
            }
            rewinddir(data_dir);
                ...
                if (ST_DIR[0] == 'M' && ST_DIR[1] == 'u') {
                  ...
                  while ((st_ent = readdir(st_dir)) != NULL) {
                    if (st_ent->d_type == DT_REG) {
                      ...
                      sprintf(RSfname, "%s%s/%s", RS_DIR, data_ent->d_name,
                              st_ent->d_name);
                      if ((MuFile = fopen(fname, "rb")) == NULL) {
                        printf("OPEN ERROR:\n %s\n", fname);
                      } else {
                        if ((RSMuFile = fopen(RSfname, "wb")) == NULL) {
                          ...
                          printf("RESAVE:\n %s\n", RSfname);
                          MuonResave->ResaveDataMUON(MuFile, RSMuFile,
                                                     atoi(Nst));
                        }
                      }
                    }
...
int main(int argc, const char *argv[]) {
  ...
  if (!DATA_LIST) {
    ...
    while (getline(DATA_LIST, data)) {
      if (MuonResave->GetMode() == 0) ReadGRANDE(data, MuonResave);
