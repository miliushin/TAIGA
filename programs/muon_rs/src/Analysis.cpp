...
Analysis::~Analysis(){
    ...
    if((FStatus= fopen(status_file_name,"w")) !=NULL){
        ...
        for(int i=0;i<NUMBER_OF_MUON_ST;i++){
            ...
            fprintf(FStatus, "     %d    %d(100%%)           %d(%f%%)\n",i,NeventMUON[i],NeventMUON_NEW[i],PERevent);
        }
        fclose(FStatus);
    }
    else{
...
int Analysis::ResaveDataMUON(FILE *FileMu, FILE *FileMuNew, int Nstation){
    ...
    while (i_File < END_File) {
        ...
        TimeEvent = GetTimeMUON(buf + 12) + TimeDelayMUON[Nstation-1]; // - 10. before 10.12.2022
...
int Analysis::ReadTimeGRANDE(FILE *File_GRANDE_tim){
    ...
    while(!feof(File_GRANDE_tim)){
        ...
        fscanf(File_GRANDE_tim,"%d%d%d%d%s%s",&Nst,&i1,&i2,&i3,strTime,strDelay);
        ...
        double mls = 100.*chd(strTime,9) + 10.*chd(strTime,10) + chd(strTime,11);
        double mks = 100.*chd(strTime,13) + 10.*chd(strTime,14) + chd(strTime,15);
        double ns  = 100.*chd(strTime,17) + 10.*chd(strTime,18) + chd(strTime,19);
        ...
        TimeGRANDE[Nst-31][NeventGRANDE[Nst-31]] = 3600.*h + 60.*m + s + mls/1000. +
          mks/1000000. + ns/1000000000. + atof(strDelay_tmp);
        NeventGRANDE[Nst-31]++;
...
void Analysis::ReadTimeMUON(FILE *FileMu, int Nstation){
    ...
    while (i_File < END_File) {
        ...
        TimeMUON[Nstation-1][NeventMUON[Nstation-1]] = GetTimeMUON(buf + 12) +
